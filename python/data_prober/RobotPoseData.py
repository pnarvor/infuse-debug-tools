...
class RobotPoseData:
    ...
    def display(self, verbose=False, blocking=False):
        ...
        axes[0].plot(self.gpsLtfTr[:,0], self.gpsLtfTr[:,1], '--o', label="GPS LTF", markeredgewidth=0.0)
        axes[0].plot(self.odometryLtfTr[:,0], self.odometryLtfTr[:,1], '--o', label="Odometry LTF", markeredgewidth=0.0)
        axes[0].plot(self.tokamakTr[:,0], self.tokamakTr[:,1], '--o', label="Tokamak")
        axes[0].plot(self.robotLtfTr[:,0], self.robotLtfTr[:,1], '--o', label="Robot pose recomposed", markeredgewidth=0.0)
        axes[0].legend(loc="upper right")
        axes[0].set_xlabel("East (m)")
        axes[0].set_ylabel("North (m)")
        axes[0].set_aspect('equal')
