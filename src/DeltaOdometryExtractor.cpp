...
namespace infuse_debug_tools
{
    ...
    DeltaOdometryExtractor::DeltaOdometryExtractor(const std::string &output_dir, const std::vector<std::string> &bag_paths, const std::vector<std::string> &pose_topics, const std::string &output_name)
    : output_dir_{output_dir},
      ...
      last_pose_ptr_{std::make_unique<asn1SccTransformWithCovariance>()}
    {}

    void DeltaOdometryExtractor::Extract()
    {
    ...
    void DeltaOdometryExtractor::ProcessPose(const infuse_msgs::asn1_bitstream::Ptr &msg)
    {
        ...
        if(!has_last_pose_)
        {
            // memorize first pose then exit function;
            // log starts at next iteration where delta can be computed
    ...
    std::pair<std::string, size_t> DeltaOdometryExtractor::GetMainTopicInfo()
    {
        size_t n_poses = 0;
        size_t main_idx = 0;
        size_t idx = 0;
    ...
    void DeltaOdometryExtractor::ComputeDelta()
    {
        ...
        q0.w() = last_pose_ptr_->data.orientation.arr[3];

        q1.x() = asn1_pose_ptr_->data.orientation.arr[0];
        q1.y() = asn1_pose_ptr_->data.orientation.arr[1];
